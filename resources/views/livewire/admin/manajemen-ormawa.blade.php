    ...
    @if($viewMode === 'card')
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($ormawas as $ormawa)
                <div wire:key="ormawa-card-{{ $ormawa->id }}" class="group bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                ...
                <table class="w-full text-sm text-left text-slate-600">
                    ...
                    <tbody>
                        @forelse($ormawas as $ormawa)
                            <tr wire:key="ormawa-row-{{ $ormawa->id }}" class="bg-white border-b border-slate-100 hover:bg-blue-50/30 transition-colors">
                                <td class="px-6 py-4">
                                    @if($ormawa->logo)
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" @click.away="if(!$event.target.closest('.modal-content')) batal()">
                ...
                <form wire:submit="save" class="p-6 space-y-5"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input wire:model="logo" type="file" accept="image/*"
                                class="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer disabled:opacity-50" wire:loading.attr="disabled" wire:target="logo">
                        </div>
                        @error('logo') <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p> @enderror

                        <div x-show="uploading" x-cloak class="mt-4">
                            <div class="flex items-center justify-between text-xs mb-1.5">
                                <span class="font-semibold text-slate-600 inline-flex items-center gap-2">
                                ...
                                <span class="font-mono text-blue-600" x-text="progress + '%'"></span>
                            </div>
                            <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-full bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full transition-all duration-300" :style="`width: ${progress}%`"></div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2 border-t border-slate-100">
                        ...
                        <button type="submit" wire:loading.attr="disabled" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-[0.97] text-white text-sm font-semibold rounded-xl transition-all shadow-sm disabled:opacity-50 inline-flex items-center gap-2">
                            <svg wire:loading wire:target="save" style="display: none;" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                            <span wire:loading.remove wire:target="save">{{ $editId ? 'Simpan Perubahan' : 'Tambah ' . ($pageType === 'ukm' ? 'UKM' : 'Ormawa') }}</span>
