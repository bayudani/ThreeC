                ...
                <form wire:submit.prevent="uploadDokumen" class="p-6 space-y-5"
                    ...
                    @if($file_bukti && !$errors->has('file_bukti'))
                        <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                            ...
                            @endphp
                            @if($tmpIsImage)
                                <img src="{{ $file_bukti->temporaryUrl() }}" class="w-14 h-14 object-cover rounded-md border border-slate-200 shrink-0">
                            @else
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-slate-700 truncate">{{ $file_bukti->getClientOriginalName() }}</p>
                                <p class="text-xs text-slate-400">{{ round($file_bukti->getSize() / 1024) }} KB • {{ strtoupper($tmpExt) }}</p>
                            </div>
                            <span class="text-xs font-semibold text-emerald-600">Siap</span>
                        </div>
                    @endif
